...
sumChain (Block c x) = sumChain c + x 
...
maxChain (Block c x) = if x > others then x else others
    where
        others = maxChain c
...
longerChain c1 c2 = if c1Len >= c2Len then c1 else c2
    where
        c1Len = lengthChain c1
        c2Len = lengthChain c2
...
validChain (Block c x) = (x > maxChain c) && validChain c
...
takeN :: Int -> Chain txs -> Chain txs
takeN _     GenesisBlock = GenesisBlock
takeN n (Block c txs)
  | n <= 0               = GenesisBlock
  | n >= chainLength     = c |> txs 
  | n == chainLength - 1 = c
...
isPrefixOf c1 c2 = eqChain c1 (takeN (lengthChain c1) c2)
...
commonPrefix c1 c2
  | isPrefixOf c1 c2 = c1
  | otherwise        = commonPrefix (takeN (lengthChain c1 - 1) c1) c2
...
hasBlockProp f (Block c txs) = if f txs then True else hasBlockProp f c
...
hasBlock txs c = hasBlockProp (== txs) c
...
uniqueBlocksMemo :: Eq txs => Chain txs -> Chain txs -> Bool
uniqueBlocksMemo _ GenesisBlock  = True
uniqueBlocksMemo m (Block c txs) = if hasBlock txs m then False else uniqueBlocksMemo (m |> txs) c

uniqueBlocks :: Eq txs => Chain txs -> Bool
uniqueBlocks = uniqueBlocksMemo GenesisBlock
...
propUniqueBlocks5 :: Bool
propUniqueBlocks5 = and [ 
    propUniqueBlocks1,
    propUniqueBlocks2,
    propUniqueBlocks3,
    propUniqueBlocks4
...
allBlockProp f (Block c txs) = if f txs then allBlockProp f c else False
...
propAllBlockProp4 :: Bool
propAllBlockProp4 = and [ 
    propAllBlockProp1,
    propAllBlockProp2,
    propAllBlockProp3
  ]
...
maxChains [] = 0
maxChains xs = last $ sort $ map lengthChain xs
...
longestCommonPrefix c1 (c2:xs) = longestCommonPrefix (commonPrefix c1 c2) xs
...
reverseChain :: Chain Int -> Chain Int -> Chain Int
reverseChain acc GenesisBlock = acc
reverseChain acc  (Block c x) = reverseChain (acc |> x) c

balancesChainFromReversed :: Chain Int -> Chain Int -> Chain Int
balancesChainFromReversed acc GenesisBlock = acc
balancesChainFromReversed GenesisBlock (Block c x) = balancesChainFromReversed (GenesisBlock |> x) c
balancesChainFromReversed (Block acc prev) (Block c x) = balancesChainFromReversed (acc |> prev |> prev + x) c

balancesChain :: Chain Int -> Chain Int
balancesChain GenesisBlock = GenesisBlock
balancesChain c = balancesChainFromReversed GenesisBlock (reverseChain GenesisBlock c)
...
shortenWhile f (Block c txs) = if f txs then shortenWhile f c else c |> txs
...
buildAcc :: Chain Int -> Int -> Chain Int
buildAcc c            0 = c
buildAcc GenesisBlock n = buildAcc (GenesisBlock |> 1) (n - 1)
buildAcc (Block c x)  n = buildAcc (c |> x |> x + 1) (n - 1)

build :: Int -> Chain Int
build n
  | (n <= 0)  = GenesisBlock
  | otherwise = buildAcc GenesisBlock n
...
replicateChainAcc :: Chain txs -> Int -> txs -> Chain txs
replicateChainAcc c 0 _ = c
replicateChainAcc c n txs = replicateChainAcc (c |> txs) (n - 1) txs

replicateChain :: Int -> txs -> Chain txs
replicateChain n txs
  | (n <= 0) = GenesisBlock
replicateChain n txs = replicateChainAcc GenesisBlock n txs
